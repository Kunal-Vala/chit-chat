...
export const sendFriendRequest = async (req: AuthenticatedRequest, res: Response): Promise<Response | void> => {
    try {
        ...
        const incomingRequest = currentUser?.friendRequests?.find(
            (req: FriendRequest) => req.from.toString() === targetUserId && req.status === 'pending'
        );
...
export const acceptFriendRequest = async (req: AuthenticatedRequest, res: Response): Promise<Response | void> => {
    try {
        ...
        const friendRequestExists = currentUser?.friendRequests?.some(
            (req: FriendRequest) => req.from.toString() === requesterId && req.status === 'pending'
        );
...
export const getFriendsList = async (req: AuthenticatedRequest, res: Response): Promise<Response | void> => {
    try {
        ...
        const onlineCount = friends.filter((friend: Partial<IUser>) => friend.onlineStatus).length;
...
export const getFriendRequests = async (req: AuthenticatedRequest, res: Response): Promise<Response | void> => {
    try {
        ...
        const pendingRequests = currentUser.friendRequests
            ...
            .sort((a: FriendRequest, b: FriendRequest) => (b.createdAt?.getTime() || 0) - (a.createdAt?.getTime() || 0)) || [];
...
export const checkFriendshipStatus = async (req: AuthenticatedRequest, res: Response): Promise<Response | void> => {
    try {
        ...
        const areFriends = currentUser.friends?.some((friendId: string) => friendId.toString() === targetUserId);
        if (areFriends) {
        ...
        const sentRequest = targetUser.friendRequests?.find(
            (req: FriendRequest) => req.from.toString() === currentUserId && req.status === 'pending'
        );
        if (sentRequest) {
